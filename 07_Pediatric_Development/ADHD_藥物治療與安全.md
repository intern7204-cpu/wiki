---
title: ADHD 藥物治療與安全
created: 2026-04-24
updated: 2026-04-24
type: clinical
domain: [pediatric, neurodevelopmental, pharmacology]
tags: [ADHD, stimulant, nonstimulant, cardiac_safety, growth]
sources:
  - 10_來源摘要/Attention_deficit_hyperactivity_disorder_in_children_and_adolescents_Treatment_with_stimulant_medications.md
  - 10_來源摘要/Attention_deficit_hyperactivity_disorder_in_children_and_adolescents_Treatment_with_nonstimulant_medications.md
  - 10_來源摘要/Cardiac_evaluation_of_patients_receiving_pharmacotherapy_for_attention_deficit_hyperactivity_disorder_ADHD.md
  - 10_來源摘要/Attention_deficit_hyperactivity_disorder_in_children_and_adolescents_Overview_of_treatment_and_prognosis.md
source_tier: 1
evidence_level: consensus
confidence: high
contested: true
contradictions:
  - stimulant 雖是 first-line，但不是零風險或永遠足夠的單獨解法。
  - nonstimulant 並非一律比較安全、比較適合或比較快見效。
  - routine ECG 不是所有 children starting ADHD medication 的 blanket requirement。
---

# ADHD 藥物治療與安全

## 一句話定義

ADHD pharmacotherapy 的核心不是「選一顆藥」，而是 **who needs medication, which class fits the problem, and how to monitor growth / sleep / cardiovascular / psychiatric risk over time**。

## 核心機制

- 目前頁面尚未把已知與機制拆開；需回源補成可教學的病理生理或方法原理。

## 臨床表現

### 何時考慮 medication

- 當 ADHD 造成明顯 functional impairment，且 psychosocial / school support 不足以達標時，應考慮 medication。
- 選藥前先確認 diagnosis、coexisting condition、family preference 與日常 impairment pattern。

### stimulant：多數需要 medication 的 first-line

- methylphenidate 與 amphetamine 類都可用。
- 優勢是 onset 快、平均 efficacy 較強。
- 但要主動監測 appetite、weight / growth、sleep、HR / BP、mood lability、psychosis / mania signal、diversion / misuse。

### nonstimulant：條件式選項

- 常見情境包括 stimulant response 不佳、side effect 無法接受、contraindication、family preference。
- alpha-2 agonist 對 tic、sleep difficulty、ASD、較年幼個案常特別有用。
- atomoxetine、viloxazine 起效較慢，且各有 CYP interaction 與 BP / HR / suicidality monitoring 重點。

### pretreatment safety screen

- cardiovascular history：syncope、palpitation、exercise intolerance、known heart disease、family history of sudden death / arrhythmia。
- 也要看 baseline BP / HR、height / weight、sleep、tics、mood、bipolar risk、substance use。
- 若有特定 cardiac red flag，再決定 targeted ECG / cardiology referral；不是 indiscriminate screening。

### 追蹤重點

- titration 期需要更密集追蹤。
- 學校回饋與家庭觀察都要納入，否則很容易錯估藥效。
- drug holiday 只能在功能仍可接受、且治療目標允許時討論。

### 臨床提醒

- 「比較不刺激」不等於風險更低；nonstimulant 也可能有重要 side effect。
- tics、anxiety、substance-use concern 多半是 individualized decision，而不是永遠不用 stimulant。

## 評估方式

- 目前頁面尚未整理出 History、Physical examination、Scale / test、Imaging / lab 的實際用法。

## 治療原則

- 目前頁面尚未整理出 Non-pharmacologic、Pharmacologic、Injection / procedure、Rehabilitation program 的決策順序。

## 臨床決策點

- 什麼情況要治療？先看是否真的改變 pain、function、safety、participation 或訓練輸出。
- 什麼情況要轉介？遇到 red flags、診斷不確定、需要程序性介入或超出本頁可處理範圍時。
- 什麼情況不該做？當證據不足、機制不合、風險高於預期收益，或結果不會改變決策時。

## 限制與未定論

### frontmatter contradictions

- stimulant 雖是 first-line，但不是零風險或永遠足夠的單獨解法。
- nonstimulant 並非一律比較安全、比較適合或比較快見效。
- routine ECG 不是所有 children starting ADHD medication 的 blanket requirement。

## 理解缺口

- ADHD 藥物治療與安全 最容易和哪幾個 mimic / differential 混淆？差異機制是什麼？
- 哪些 bedside finding 真的會改變 treatment threshold，而不是只增加診斷把握？
- 哪些量表或影像異常和功能障礙不一定一致？原因是什麼？
- 目前缺少完整評估框架：History、Physical examination、Scale / test、Imaging / lab 仍需補強。
- 目前缺少明確處置順序：何時觀察、何時介入、何時轉介仍需補強。

## 臨床使用版

- 若病人的症狀、檢查與功能限制符合 ADHD 藥物治療與安全 的框架，先用本頁的 History / Physical / red flags 決定是否需要影像、介入或轉介。
- 若只有名詞相像、但機制或時序對不上，先回 differential，不要直接套用診斷與治療。

## 來源

### 來源摘要連結

- [[10_來源摘要/Attention_deficit_hyperactivity_disorder_in_children_and_adolescents_Treatment_with_stimulant_medications]]
- [[10_來源摘要/Attention_deficit_hyperactivity_disorder_in_children_and_adolescents_Treatment_with_nonstimulant_medications]]
- [[10_來源摘要/Cardiac_evaluation_of_patients_receiving_pharmacotherapy_for_attention_deficit_hyperactivity_disorder_ADHD]]
- [[10_來源摘要/Attention_deficit_hyperactivity_disorder_in_children_and_adolescents_Overview_of_treatment_and_prognosis]]

### 證據標記

- 來源層級：1
- evidence_level：consensus
- confidence：high

## 相關頁面

### 相關頁面

- [[ADHD_治療總論]]
- [[ADHD_評估與診斷]]
- [[ADHD_睡眠與常見共病]]
- [[../10_來源摘要/Attention_deficit_hyperactivity_disorder_in_children_and_adolescents_Treatment_with_stimulant_medications]]
- [[../10_來源摘要/Attention_deficit_hyperactivity_disorder_in_children_and_adolescents_Treatment_with_nonstimulant_medications]]
- [[../10_來源摘要/Cardiac_evaluation_of_patients_receiving_pharmacotherapy_for_attention_deficit_hyperactivity_disorder_ADHD]]
