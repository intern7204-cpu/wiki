---
title: Cerebral Palsy 總論
created: 2026-04-24
updated: 2026-04-26
type: clinical
domain: [pediatric, neurodevelopmental, rehabilitation, PMR]
tags: [cerebral_palsy, GMFCS, comorbidity, participation, tone, transition]
sources:
  - 09_來源摘要/Cerebral_Palsy_and_Other_Motor_Disorders.md
  - 09_來源摘要/Consequences_of_Preterm_Birth.md
  - 09_來源摘要/Detailed_neurologic_assessment_of_infants_and_children.md
source_tier: 1
evidence_level: consensus
confidence: high
contested: true
contradictions:
  - CP is a nonprogressive brain injury, but the child's phenotype and musculoskeletal burden change with growth.
  - Normal MRI does not exclude CP; genetic and neurodegenerative mimics remain important.
---

# Cerebral Palsy 總論

## 一句話定義

Cerebral Palsy (CP) 是 **developing fetal or infant brain 的非進行性損傷所導致的 movement and posture disorder with activity limitation**，但它的臨床問題從來不只 motor 本身。

## 核心機制

### 核心概念

- CP diagnosis 需要：
  - nonprogressive motor abnormality
  - developmental history 支持
  - serial neurologic examination
  - 必要時 neuroimaging / genetics
- 早期線索包含：
  - prolonged primitive reflexes
  - asymmetry
  - early handedness
  - poor suck / feeding difficulty
  - delayed gross motor milestone

### 分類框架

- 傳統分類看 movement type 與 topography：
  - spastic
  - dyskinetic
  - ataxic
  - hemiplegia / diplegia / quadriplegia 等
- 現代臨床更需要 functional classification：
  - GMFCS
  - MACS
  - EDACS
  - CFCS
  - VFCS
- 重點是描述 **usual performance**，不是單次門診 capability。

### 方法學重點

- MRI 常有幫助，但 normal MRI 不能直接排除 CP。
- 正常或 atypical imaging 時，要提高對 genetic/metabolic mimic 的警覺。
- active, goal-directed therapy 比 passive modality 更符合 rehabilitation logic。
- orthoses、wheelchair、communication device、feeding plan 都屬於核心處方，不是附屬品。

## 臨床表現

### associated conditions

- epilepsy
- dysphagia / drooling / feeding problem
- constipation / bladder-bowel issue
- hip displacement / scoliosis / contracture / pain
- visual impairment / hearing impairment
- intellectual disability / ADHD / ASD / anxiety / depression
- sleep problem、fatigue、participation restriction

### 臨床或研究上的重要性

- CP 不是單一 tone problem；它是 lifelong rehabilitation condition。
- 早期 diagnosis 的價值，在於 early intervention、family support 與 equipment planning 可以更早啟動。
- adolescents / adults with CP 仍會面臨 pain、fatigue、mobility decline、sexuality 與 transition issue。

## 評估方式

- 目前頁面尚未整理出 History、Physical examination、Scale / test、Imaging / lab 的實際用法。

## 治療原則

- 目前頁面尚未整理出 Non-pharmacologic、Pharmacologic、Injection / procedure、Rehabilitation program 的決策順序。

## 臨床決策點

- 什麼情況要治療？先看是否真的改變 pain、function、safety、participation 或訓練輸出。
- 什麼情況要轉介？遇到 red flags、診斷不確定、需要程序性介入或超出本頁可處理範圍時。
- 什麼情況不該做？當證據不足、機制不合、風險高於預期收益，或結果不會改變決策時。

## 限制與未定論

### 目前限制 / 爭議點

- CP umbrella 下異質性高，單頁只能做骨架，無法取代各子主題專頁。
- tone management、hip surveillance、feeding/swallowing、mental health 與 adult transition 都需要後續獨立頁。

### frontmatter contradictions

- CP is a nonprogressive brain injury, but the child's phenotype and musculoskeletal burden change with growth.
- Normal MRI does not exclude CP; genetic and neurodegenerative mimics remain important.

## 理解缺口

- Cerebral Palsy 總論 最容易和哪幾個 mimic / differential 混淆？差異機制是什麼？
- 哪些 bedside finding 真的會改變 treatment threshold，而不是只增加診斷把握？
- 哪些量表或影像異常和功能障礙不一定一致？原因是什麼？
- 目前缺少完整評估框架：History、Physical examination、Scale / test、Imaging / lab 仍需補強。
- 目前缺少明確處置順序：何時觀察、何時介入、何時轉介仍需補強。

## 臨床使用版

- 若病人的症狀、檢查與功能限制符合 Cerebral Palsy 總論 的框架，先用本頁的 History / Physical / red flags 決定是否需要影像、介入或轉介。
- 若只有名詞相像、但機制或時序對不上，先回 differential，不要直接套用診斷與治療。

## 來源

### 來源摘要連結

- [[09_來源摘要/Cerebral_Palsy_and_Other_Motor_Disorders]]
- [[09_來源摘要/Consequences_of_Preterm_Birth]]
- [[09_來源摘要/Detailed_neurologic_assessment_of_infants_and_children]]

### 證據標記

- 來源層級：1
- evidence_level：consensus
- confidence：high

## 相關頁面

### CP 子頁（依 Feynman 骨架單一概念）

- [[CP_早期辨識與診斷]]
- [[CP_功能分類系統]]
- [[CP_髖部監測]]
- [[CP_張力管理]]
- [[CP_呼吸照護]]
- [[CP_骨骼健康與骨折]]

### 相關頁面

- [[小兒復健評估]]
- [[兒童神經學評估]]
- [[早產兒長期結局]]
- [[兒童視覺障礙與CVI]]
- [[../03_疾病與臨床主題/Spasticity_概論]]
- [[../06_Gait_Biomechanics/下肢矯具總論]]
- [[../03_疾病與臨床主題/輪椅與座位系統總論]]
- [[../03_疾病與臨床主題/吞嚥障礙復健總論]]
- [[../09_來源摘要/Cerebral_Palsy_and_Other_Motor_Disorders]]
