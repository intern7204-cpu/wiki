...
tags: [EDX, EMG, NCS, referral_question, diagnostic_reasoning]
sources:
...
高品質 EDX 不是先做檢查再想問題，而是先用 symptom distribution、weakness、sensory loss 與 reflex pattern，寫出能改變 management 的 referral question。
...
- referral question 若太含糊，得到的往往只是很多數值，而不是可用的臨床結論。
...
- 即使問題設計良好，sensitivity 仍會受 disease timing、background polyneuropathy 與 false positive 影響。
