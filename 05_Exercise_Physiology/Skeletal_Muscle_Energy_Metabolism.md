...
骨骼肌運動代謝的核心，不是某一條路徑單獨主宰，而是 **PCr、glycolysis、oxidative phosphorylation** 三條路徑在不同強度與時間尺度上的動態協調。
...
|------|------|--------------|
| PCr | 供能最快、容量最小 | 秒級 |
| Glycolysis | 供能快、會伴隨 lactate 與 H+ 動態 | 十秒到數分鐘 |
| Oxidative phosphorylation | 供能最可持續 | 分鐘到小時 |
...
- Lactate 不是廢物。
- 它是可氧化底物、糖質新生前驅物，也是 interorgan signaling 分子。
- 這和 [[Lactate_Shuttle]] 的框架一致。

### Muscle fiber type 的解讀必須精確

- 不能把 MHC type 直接等同於 oxidative capacity。
...
- Kemp 1993 顯示 PCr recovery 對 ADP 呈飽和動力學，並可作為 mitochondrial function 的活體指標。
- 這讓高強度運動的恢復，不再只是 performance 現象，而有明確代謝基礎。

### 與訓練適應的關係

- Endurance training 會提升 mitochondrial density、capillary supply、substrate handling 與 recovery kinetics。
- 結果常表現在：
  - LT / GET 右移
...
- skeletal muscle 也是 secretory organ。
- 運動後除了代謝通量變化，還會出現 myokine / muscle-organ signaling。
- 目前 human evidence 最強的是 IL-6；其他多數 myokine pathway 仍屬 emerging evidence。
- 這部分見 [[Myokines_與_Muscle_Organ_Crosstalk]]。

### Training Intensity Distribution（TID）不是純行銷名詞

- 不同 TID 的本質，是不同域內 volume 的配置。
- 但 Oliveira 2024 顯示，**Polarized training 並非對所有結果全面 superior**。
- 因此代謝適應要從 domain 與目標任務出發，不要反過來把某個 TID 名稱當成機制。
...
- Skeletal Muscle Energy Metabolism during Exercise 和最相近、最常被混用的概念差在哪？
...
- 若要把 Skeletal Muscle Energy Metabolism during Exercise 用在 bedside 或運動處方，先確認它回答的是哪一個機制或強度邊界，再決定能否改變評估與處置。
